...
export class Graph {
  ...
  public __uniqval__:number;
  public _lookup: any;
  public _nodes: any;
  public _edges: any;
  public _nodeCollections: any;
  public _edgeCollections: any;
  ...
  createEdge(entity, properties?: any) {
  ...
  trace(fromNode, toNode, direction?: number) {

    let passCondition = function(node) {
      return node === toNode;
    };

    return this._search(fromNode, passCondition, 1, direction, undefined, undefined)[0] || new Path([]);
  ...
  _search(node, passCondition, count, direction, minDepth, maxDepth) {
    ...
    function orderedSetInsert(arr, val) {
      ...
      // TODO: test
      let bool = (arr[i] < val) ? 1 : 0;

      return arr.splice(i + bool, 0, val);
